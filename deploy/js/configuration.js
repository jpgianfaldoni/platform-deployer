(function (root, factory) {
  ...
  function normalize(input = {}) {
    ...
    const projectPrefix = String(input.project_prefix || 'databricks-workspace').trim();
    const requestedNetworkConfiguration = String(input.network_configuration || 'standard');
    const natGatewayMode = String(input.nat_gateway_mode ||
      (privateLink && requestedNetworkConfiguration === 'fully_private' ? 'none' : 'single'));
    const networkConfiguration = privateLink
      ? requestedNetworkConfiguration === 'custom' ? 'custom' : natGatewayMode === 'none' ? 'fully_private' : 'standard'
      : requestedNetworkConfiguration;
    const config = {
      schemaVersion: 1,
      provider: 'aws',
      project_prefix: projectPrefix,
      resource_prefix: projectPrefix,
      ...
      security_group_ids: privateLink ? toList(input.security_group_ids) : [],
      ...
      tags: {},
      new_security_group_name: `${projectPrefix}-databricks-sg`,
      sg_egress_ports: [...(privateLink ? PRIVATELINK_PORTS : STANDARD_PORTS)],
      additional_egress_ips: [],
      metastore_mode: String(input.metastore_mode || 'new'),
      metastore_id: String(input.metastore_id || '').trim(),
      metastore_name: String(input.metastore_name || `${projectPrefix}-metastore`).trim(),
      new_catalog: false,
      catalog_name: '',
    ...
    if (managed && CIDR.isValid(config.vpc_cidr_range)) {
      try {
        const network = CIDR.allocate(config.vpc_cidr_range, config.availability_zones, config.subnet_prefix, {
          publicSubnets: config.nat_gateway_mode !== 'none',
  ...
  function validate(input) {
    ...
    const managed = !config.enable_private_link
      ? config.network_mode === 'managed'
      : ['standard', 'fully_private'].includes(config.network_configuration);
    if (managed && !['single', 'per_az', 'none'].includes(config.nat_gateway_mode)) {
      add('nat_gateway_mode', 'Choose a supported NAT gateway option.');
    }
    if (managed && config.nat_gateway_mode === 'none' && !config.enable_private_link) {
      add('enable_private_link', 'Backend PrivateLink is required when no NAT gateway is selected.');
    }
    if (managed) {
